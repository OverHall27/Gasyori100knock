...
def LaplacianFilter(_img, K_size=3):
    ...
    tmp = result.copy()
    tmp[pad: pad + Hol, pad: pad + Ver] = _img.copy().astype(np.float)
    ...
    for x in range(Hol):
        for y in range(Ver):
            result[pad + x, pad + y] = np.sum(tmp[x: x + K_size, y: y + K_size] * kernel_lap)
...
result = LaplacianFilter(gray_img, K_size=3)

cv2.imwrite("myans_17.jpg", result)
