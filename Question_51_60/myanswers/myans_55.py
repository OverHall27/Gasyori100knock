...
def TemplateMatchSAD(obj_img, temp_img):
    ...
    for sx in range(Hor - tHor):
        for sy in range(Ver - tVer):
            ssd[sy, sx] = np.sum(np.abs(obj_img[sy:sy+tVer, sx:sx+tHor] - temp_img))
...
result = TemplateMatchSAD(obj, temp)

cv2.imwrite("myans_55.jpg", result)
