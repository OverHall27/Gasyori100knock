...
def Morphological(binary, times=1, mode='expansion'):

    def expansion(binary):
        Ver, Hor = binary.shape

        fileter = list(map(np.uint8, [[0, 1, 0], [1, 0, 1], [0, 1, 0]]))
...
def Opening(binary, times=1):
    binary = Morphological(binary, times, 'shrink')
    binary = Morphological(binary, times, 'expansion')
    return binary
    
def Closing(binary, times=1):
    binary = Morphological(binary, times, 'expansion')
    binary = Morphological(binary, times, 'shrink')
    return binary
...
gray = BGRTOGRAY(img)
nichika = OtsuThreshould(gray)
binary = nichika.copy()

binary = Clng(binary, 3)
result = np.abs(binary - nichika)
cv2.imshow("result", result)
cv2.imwrite("myans_53.jpg", result)
cv2.waitKey(0)
