...
def HSVColorTracking(hsv, hmin, hmax):
    Ver, Hor, _ = hsv.shape
    img = np.zeros((Ver, Hor))
    for x in range(Hor):
        for y in range(Ver):
            if hsv[y, x, 0] < hmin or hsv[y, x, 0] > hmax:
                img[y, x] = 0
            else:
                img[y, x] = 255
...
##青色抽出 -> 180 <= H <= 260
result = HSVColorTracking(hsv, 180, 260)

cv2.imwrite("myans_70.png", result)
